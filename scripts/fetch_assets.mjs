...
const manifestPath = "scripts/assets.manifest.json"; const root = process.cwd();
...
const res=[]; for(const it of m.files||[]){const dest=path.resolve(root,it.dest); try{await dl(it.url,dest); const sum=await sha(dest); if(sum!==it.sha256) throw new Error(`SHA mismatch for ${it.dest}\nexpected ${it.sha256}\nactual   ${sum}`); res.push({dest:it.dest,ok:true}); console.log("ok  "+it.dest)}catch(e){res.push({dest:it.dest,ok:false,error:String(e)}); console.error("err "+it.dest+": "+(e.message||e))}}
